...
/** Opt-in dialog relay for extensions installed alongside the bridge. A missing
 * controller fails closed; ordinary Pi UI dialogs are NOT silently intercepted. */
export async function relayRoamPiDialog(kind: "confirm" | "select" | "input", title: string, detail: string | string[]) {
  return active?.relayDialog(kind, title, detail) ?? (kind === "confirm" ? false : undefined);
}
