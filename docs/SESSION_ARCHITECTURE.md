...
Detach and close first prevent queued RPC writes from being claimed, then stop any claimed write before reporting cancellation and retiring its stream generation. Pending request failures report the active `.detached` or `.closing` lifecycle phase rather than a contradictory failed phase. They end the iOS-side PTY and SSH connection. They do not send `tmux kill-session`, terminate Pi, or delete remote state. The app does not expect continuous execution while backgrounded: iOS may suspend it. tmux is the continuity mechanism, and reconnect is explicit after suspension or transport loss.
