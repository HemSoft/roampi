...
The normative schema is [`roampi.schema.json`](roampi.schema.json). The Swift models and behavioral validator live in `RoamPiCore/Configuration`. Documents allow at most 64 nested JSON container levels, declared by `x-roampi-max-document-depth` and enforced before adoption.
