...
`npm ci --prefix Remote/RoamPiExtension && npm run typecheck --prefix Remote/RoamPiExtension && npm test --prefix Remote/RoamPiExtension` runs the protocol, permissions, lease, redaction, lifecycle, and disposable credential-free Pi RPC integration tests, plus the interactive PTY test on macOS. This uses a temporary agent directory and scripted provider, not the developer's Pi credentials, session files, or live tailnet.
